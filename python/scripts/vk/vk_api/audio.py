...
class VkAudio(object):
    ...
    def get_iter(self, owner_id=None, album_id=None, access_hash=None):
        ...
        audio_count = self._vk.method("users.get",values={"user_ids":owner_id,"fields":"counters"})[0].get("counters").get("audios")
        count = Counter(audio_count)
    ...
    def get_iter_whithout_urls(self, owner_id=None, album_id=None, access_hash=None):
        """ Получить список аудиозаписей пользователя (по частям)
        ...
        while True:
            ...
            tracks = scrap_data_without_urls(
                response.text,
                self.user_id,
                filter_root_el={'class_': 'audioPlaylist__list'} if album_id else None
            )
    ...
    def get_without_urls(self, owner_id=None, album_id=None, access_hash=None):

        return list(self.get_iter_whithout_urls(owner_id, album_id, access_hash))
...
class Counter(object):
    ...
    def __init__(self,max,start=0,message="{}/{}",state=True):
        self.max = max
        self.now = start
        self.state = state
    ...
    def add(self,sum=1):
        ...
        self.now+= sum
        print(self.message.format(self.now,self.max))

def scrap_data(html, user_id, filter_root_el=None, convert_m3u8_links=True, http=None, count=Counter(0,state=False)):
...
def scrap_data_without_urls(html, user_id, filter_root_el=None):
    """ Парсинг списка аудиозаписей из html страницы """
